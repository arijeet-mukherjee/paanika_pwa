...
function ProductList() {
	...
	async function getallproducts(i, fromPrice, toPrice, catId) {
		...
		Util.apiCall(
			"GET",
			Util.baseUrl,
			`products?getCategory=1&getDetail=1&productCategories=${
				catId ? catId : categoryId
			}&stock=1&limit=4${queryParam}${queryParamPriceRange}`,
			Util.header
		)
			.then((dt) => {
	...
	function getallCategories() {
		Util.apiCall(
			"GET",
			Util.baseUrl,
			"category?limit=200000&sortBy=id&sortType=ASC&getDetail=1&getGallary=1",
			Util.header
		)
			.then((s) => {
				setCategories(s);
			...
			<div className="h-scroller d-block d-lg-none">
				<nav className="nav h-underline">
					{categoriesData.map((v, i) => {
						return (
							<div key={v.id} className="h-link me-2">
								<Link
									to={"/category/" + v.id}
					...
					<div className="d-flex flex-column h-100">
						...
						{isLoading ? (
							...
							>
								{allProducts && allProducts.length > 0
									? [...allProducts].map((e, i) => {
											if (viewType.grid) {
												return <Productcard key={i} products={e} />;
											}
											return <Productcard key={i} products={e} />;
									  })
									: ""}
						...
						<div className="d-flex align-items-center mt-auto">
							<span className="text-muted small d-none d-md-inline">
								Showing {page + 1} of {totalPage ? totalPage : 1}
							</span>
